...
public class FileController extends BaseController{
    ...
    @ResponseBody
    public JsonResult getVideosName(HttpServletRequest request,HttpServletResponse response)throws ServletException,IOException{
    ...
    @ResponseBody
    public JsonResult uploadVideo(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    ...
    @ResponseBody
    public JsonResult uploadTxt(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
...
//初始化ascClient,暂时不支持多region（请勿修改）
    ...
    public ResponseEntity<byte[]> download() throws IOException{
        System.out.println("调用了download");
        File file=new File("D:\\aiadImage\\ad.mp4");
        HttpHeaders httpHeaders=new HttpHeaders();
        String fileName=new String("ad.mp4".getBytes("UTF-8"),"iso-8859-1");
        httpHeaders.setContentType(MediaType.APPLICATION_OCTET_STREAM);
        return new ResponseEntity<byte[]>(FileUtils.readFileToByteArray(file),httpHeaders, HttpStatus.OK);
    }

    @RequestMapping("/downloadVideo2")
